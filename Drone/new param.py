...
PARAM_CATEGORIES = {
    "Stabilize Roll (Error to Rate)": ["STB_RLL_P", "ACCEL_MA_RLL"],
    "Stabilize Pitch (Error to Rate)": ["STB_PIT_P", "ACCEL_MA_PIT"],
    "Stabilize Yaw (Error to Rate)": ["STB_YAW_P", "ACCEL_MA_YAW"],

    "Position XY (Dist to Speed)": ["POS_XY_P", "POS_XY_TC"],
    ...
    "Velocity XY (Vel to Accel)": ["VEL_XY_P", "VEL_XY_I", "VEL_XY_D", "VEL_XY_IMAX"],

    "Throttle Accel (Accel to motor)": ["THR_ACCEL_P", "THR_ACCEL_I", "THR_ACCEL_D", "THR_ACCEL_IMAX"],
    "Throttle Rate (VSpd to accel)": ["THR_RATE_P"],

    "Basic Filters": ["GYRO_FILTER", "ACCEL_FILTER"],

    "Static Notch Filter": ["FLTR_ENABLE", "FLTR_FREQ", "FLTR_BW", "FLTR_ATT"],
    "Harmonic Notch Filter": ["HNTCH_ENABLE", "HNTCH_MODE", "HNTCH_REF", "HNTCH_FREQ", "HNTCH_BW", "HNTCH_ATT", "HNTCH_HMNCS"],

    "Altitude Hold (Alt to climbrate)": ["AH_P"],

    "Waypoint Navigation": ["WPNAV_SPEED", "WPNAV_RADIUS", "WPNAV_SPEED_UP", "WPNAV_SPEED_DN", "WPNAV_LOITERSPEED"]
}
...
PARAM_NAME_MAPPING = {
    ...
    "ACCEL_MA_RLL": "Accel MA Roll",
    "ACCEL_MA_PIT": "Accel MA Pitch",
    "ACCEL_MA_YAW": "Accel MA Yaw",
    ...
    'WPNAV_SPEED_DN': 'Waypoint Navigation: Speed Down',
    'WPNAV_LOITERSPEED': 'Waypoint Navigation: Loiter Speed'
}

def get_user_friendly_name(param_id):
    """Get user-friendly parameter name or return the original if not found."""
    return PARAM_NAME_MAPPING.get(param_id, param_id)

# Connect to the ArduPilot via MAVLink
try:
    connection = mavutil.mavlink_connection('COM6', baud=9600)
    print("Connected to COM9 successfully.")
except Exception as e:
    print(f"Error connecting to COM9: {e}")
    exit()

# Wait for a heartbeat from the ArduPilot
print("Waiting for heartbeat...")
try:
    connection.wait_heartbeat(timeout=10)  # Increased timeout to 10 seconds
...
while time.time() - start_time < 30:  # Allow 30 seconds to receive parameters
    message = connection.recv_match(type='PARAM_VALUE', blocking=False, timeout=1)
    if message:
...
for category in PARAM_CATEGORIES: 
    ...
    for param_id in PARAM_CATEGORIES[category]:
        if param_id in parameters:
            categorized_params[category][param_id] = parameters[param_value]


output_file = 'categorized_ardupilot_parameters.txt'
